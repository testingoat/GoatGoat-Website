...
export function Hero() {
    return (
        <section className="relative border-b border-[color:var(--border)] bg-[radial-gradient(circle_at_top,_rgba(99,102,241,0.12),_transparent_55%),_radial-gradient(circle_at_bottom_right,_rgba(16,185,129,0.12),_transparent_55%)] overflow-hidden">
            ...
            <div className="container mx-auto flex max-w-7xl flex-col gap-16 px-6 py-20 sm:px-10 sm:py-32 lg:flex-row lg:items-center relative z-10">
                ...
                <div className="flex flex-1 justify-center lg:justify-end">
                    <div className="relative mt-8 grid grid-cols-3 gap-4 lg:mt-0">
                        <PhoneMock label="Customer" delay={0.4} />
                        <PhoneMock label="Seller" delay={0.5} className="mt-12" />
                        <PhoneMock label="Delivery" delay={0.6} className="hidden sm:flex mt-24" />
                    </div>
                </div>
            </div>
        </section>
...
function PhoneMock({ label, className = "", delay = 0 }: { label: string; className?: string; delay?: number }) {
    return (
        <motion.div
